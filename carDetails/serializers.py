...
class PCNTableSerializer(serializers.ModelSerializer):
    ...
    def get_carDetails(self,obj):
        try:
            ...
            print(cardetails)
            data = [{"number_plate": i.number_plate,"car_model":i.car_model,"car_color":i.car_color,"car_location":i.car_location,"car_image":self.context['request'].build_absolute_uri(i.car_image.url)} for i in cardetails]
            return data
        except:
            return None
    ...
    def get_pcnCode(self,obj):
        try:
            pcnCode = PCNCode.objects.filter(id=obj.id)
            print(pcnCode)
